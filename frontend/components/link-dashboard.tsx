import { Copy, ExternalLink, Loader2, RefreshCcw, Search, Trash2 } from "lucide-react";
import { LinkForm } from "./link-form";
import { useLinkStore } from "@/app/store/linkStore";
import { useEffect, useState, useTransition } from "react";
import { Linki } from "@/app/type/link";
...
export function LinkDashboard(){
    const{loading,error,links,fetchLinks,fetchLinkStats,deleteLink}=useLinkStore();
            ...
            <section className="card space-y-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <h2 className="text-2xl font-semibold text-white">Your short links</h2>
                    </div>
                    <button
                      onClick={()=>fetchLinks()}
                      type="button"
                      ...
                      {filteredLinks.map((link,index)=>{
                        ...
                        return(
                            <tr key={index} className="text-slate-200">
                                <td className="px-4 py-3 font-mono text-sm text-white">{link.code}</td>
                                <td className="px-4 py-3 max-w-[240px] truncate text-slate-400">
                                    <span title={link.target_url}>{formatUrl(link.target_url)}</span>
                                </td>
                                <td className="px-4 py-3 text-right font-semibold">{link.clicks}</td>
                                <td className="px-4 py-3 text-sm text-slate-400">
                                    { link.last_clicked ? formatDistanceToNow(new Date(link.last_clicked),{addSuffix:true}):"Never"}
                                </td>
